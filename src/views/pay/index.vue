...
import { checkOrder, postOrder } from '@/api/order'
...
export default {
  ...
  computed: {
    ...
    longAddress () {
      return `${this.defaultAddress.region.province},${this.defaultAddress.region.city},${this.defaultAddress.region.region},${this.defaultAddress.detail}`
    },
    mode () {
  ...
  methods: {
    async submitOrder () {
      if (this.mode === 'cart') {
        const res = await postOrder(this.mode, { cartIds: this.cartIds, remark: this.remark })
        console.log(res)
      } else {
  ...
  created () {
    // const res = await setThatAddress()
    // console.log(res)
    this.getAddressList()
    this.getOrder()
  }
