<template>
  <div class="prodetail">
    ...
    <div class="footer">
      ...
      <div @click="addFn('buyNow')" class="btn-buy">立刻购买</div>
    </div>
...
<script>
import { getProComments, getCargoDetail } from '@/api/cargo.js'
...
export default {
  ...
  methods: {
    ...
    async getDetail () {
      const { data: { detail } } = await getCargoDetail(this.goodsId)
      this.detail = detail
      this.image = detail.goods_images
    },
    async getComments () {
    ...
    async addCart () {
      ...
      const { data: { cartTotal }, message } = await addToCart(this.goodsId, this.addCount)
